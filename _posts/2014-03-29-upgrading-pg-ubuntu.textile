...
Reflecting on these details about how pg_upgrade works by copying the old dataase, it apparently does not need to write to the old database which makes it feel pretty safe.  In fact when testing, sending an interrupt pg_upgrade and aborting while copying page files did not cause a problem.
