...
Check the docs on nitrogen and conky for configuring those items.  In short, launch nitrogen manually once and select the background images you want, the --restore flag int the script above will preserve between sessions.  

For conky, see /etc/conky where conky.conf resides and is full of options.  To set it to display to the background, set the own_window directive to no.
...
There are quite a few cool application launchers, but gnome-do seems to have the best feature set.  For a lighter alternative checkout gmrun.

bc. sudo apt-get install gnome-do

Configuring the hotkey is a little tricker.  First we remap the super key to an unused character and clear any other mappings.  Note: These are the xmodmap entries tin the windowlab_start script above.

Create a script in autokey with the following entry which invokes a short python script and assign to the super key:
